...
- **Goal**: Allow plugins to communicate asynchronously.
  - _Example:_ The `event` module emits `event:created`. The `budget` module listens for this and automatically drafts an empty budget allocation without the `event` module needing to know the `budget` module exists.
